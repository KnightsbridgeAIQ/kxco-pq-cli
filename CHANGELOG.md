...
### Added
- `kxco-pq keygen` — derive ML-DSA-65 keypair from master secret + label via HKDF
- `kxco-pq fingerprint` — compute 16-hex KID from a public key hex string or file
- `kxco-pq rotate` — generate signed rotation manifest + well-known JSON for key transitions
